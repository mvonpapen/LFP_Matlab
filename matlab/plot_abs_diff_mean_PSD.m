...
%     P_tot_on  = P_coh_on + P_inc_on + P_vc_on;
    ...
    errorbar( (1:Nb)-0.2, nanmean(dP_inc{ii}), nanstd(dP_inc{ii}), 'vr')
    hold all
    errorbar( (1:Nb), nanmean(dP_coh{ii}), nanstd(dP_coh{ii}), 'ob')
    errorbar( (1:Nb)+0.2, nanmean(dP_vc{ii}), nanstd(dP_vc{ii}), '^g')
    ...
    for i=1:Nb
        if sig_inc(i,ii)<0.01;        
            plot([i-0.03 i+0.03]-0.2,0.9*limy([2 2]),'*k');
        elseif sig_inc(i,ii)<0.05;
            plot(i-0.2,0.9*limy(2),'*k');
        end
    ...
    errorbar( 1:Nb, nanmean(dP_bip{ii}), nanstd(dP_bip{ii}), 'ks')
    set(gca,'TickLabelInterpreter','tex');
    set(gca, 'XTick', xticks);
