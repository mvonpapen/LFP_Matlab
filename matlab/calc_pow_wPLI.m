...
function calc_pow_wPLI(task, wPLI)
    ...
    for i=1:Nex
        ...
        coi = coi/nsig;
        [a,b,~,~] = size(W);
        PLI = zeros(a, b, Nch(i), Nch(i));
        for j=1:Nch(i)-1
            for k=(j+1):Nch(i)
                Wxy = squeeze(W(:,:,j).*conj(W(:,:,k)));
                PLI(:,:,j,k) = phase_lag_index(imag(Wxy), ...
