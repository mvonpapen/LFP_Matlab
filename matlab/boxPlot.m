...
% 
%   See also TAB2BOX, QUANTILE2, COLORMAP.
...
%   Copyright 2016 University of Surrey.
    ...
    properties (AbortSet)
       ...
       useMean = false              % Show mean instead of median. (inserted by mitch (18.05.16))
    end
    ...
    methods
        ...
        % BOXPLOT Draw a box plot.
        %
        %   BOXPLOT(Y) produces a box plot of the data in Y. If Y is a
        %   vector, there is just one box. If Y is a matrix, there is one
        %   box per column. If Y is an N-by-P-by-G-by-I-by-J... array then
        %   G*I*J*... boxes are plotted hierarchically for each column P;
        %   i.e. J boxes are plotted for each index of I, and J*I boxes are
        %   plotted for each index of G. For each box, the central mark is
        %   the median of the data/column, the edges of the box are the
        ...
        %   the TAB2BOX function.
        %
        %   BOXPLOT(X,Y) specifies the x-axis values for each box. X
        %   should be a vector, with as many elements as Y has columns. The
        %   default is 1:SIZE(Y,2).
        %
        %   BOXPLOT(...,'PARAMETER',VALUE) allows the plotting options to
        %   be specified when the plot is constructed.
        %
        %   Examples
        % 
        ...
        %       % arrange data
        %       [y,x,g] = tab2box(Cylinders,MPG,when);
        %       weights_boxed = tab2box(Cylinders,weights,when);
        %       
        %       % plot
        %       figure
        %       h = boxPlot(x,y,'weights',weights_boxed);
        ...
        % set useMean option (by mitch)
        
        function set.useMean(obj,val)
            assert(islogical(val) && isscalar(val),'''USEMEAN'' must be logical.')
            obj.useMean = val;
            obj.draw('useMean');
        end
    ...
    methods (Access = private)
        ...
        function drawBox(obj,subidx,gidx,subidxAll) %#ok<INUSL>
        %DRAWBOX draw boxes
            ...
            if obj.useMean
                md = obj.statistics.mean(subidx{:}); %%Changed by mitch
            end
        ...
        function drawWhiskers(obj,subidx,gidx) %#ok<INUSD>
        ...
        function drawOutliers(obj,subidx,gidx) %#ok<INUSD>
        ...
        function drawMean(obj,subidx,gidx) %#ok<INUSD>
        ...
        function drawAddPrctiles(obj,subidx,gidx) %#ok<INUSD>
        ...
        %CALCULATESTATS calculate the statistic for the box plot
            ...
            obj.statistics.median = quantile2(obj.y,.5,[],obj.method,obj.weights); % median
            ...
            for n = 1:prod(obj.outDims)
                ...
                subidxAll{1} = ':';
                if ~isempty(obj.addPrctiles)
                    obj.statistics.addPrctiles(subidxAll{:}) = quantile2(obj.y(subidxAll{:}),obj.addPrctiles(:)./100,[],obj.method,obj.weights(subidxAll{:}));
                end
                obj.statistics.PL(subidx{:}) = quantile2(obj.y(subidxAll{:}),min(obj.percentile)/100,[],obj.method,obj.weights(subidxAll{:}));
                obj.statistics.PU(subidx{:}) = quantile2(obj.y(subidxAll{:}),max(obj.percentile)/100,[],obj.method,obj.weights(subidxAll{:}));
                [obj.statistics.Q1(subidx{:}),obj.statistics.N(subidx{:})] = quantile2(obj.y(subidxAll{:}),0.25,[],obj.method,obj.weights(subidxAll{:}));
                obj.statistics.Q3(subidx{:}) = quantile2(obj.y(subidxAll{:}),0.75,[],obj.method,obj.weights(subidxAll{:}));
                ...
                if isnumeric(obj.limit)
                    upper_limit = quantile2(obj.y(subidxAll{:}),max(obj.limit)/100,[],obj.method,obj.weights(subidxAll{:}));
                    lower_limit = quantile2(obj.y(subidxAll{:}),min(obj.limit)/100,[],obj.method,obj.weights(subidxAll{:}));
                else
