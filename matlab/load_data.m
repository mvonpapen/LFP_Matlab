...
function [t, dat, rigor, CHlab, art, ndat, LFPact] = load_data ( varargin )
%% load_dat_from_DATA 
% Function to load all data from control file loaded into DATA
%
% OPTIONAL INPUT:
%
...
if ~isempty(args.rigor)
    ndat=intersect(ndat,structfind(DATA,'rigor',args.rigor));
end
...
for i=ndat;
    ...
    %% Write out data
    j=j+1;
    load(['/afs/geo/usr/neuro/DATA/' DATA(i).patient ...
    ...
    for nch=in
        ...
        if nch<=length(DATA(i).interval)-3
